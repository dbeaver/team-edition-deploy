...
5. Choose configuration for your cluster database:
   - If you plan to use the PostgreSQL internal container:
     ...
     - Open `variables.tf`.
     - Update `variables.tf` file. You must set `POSTGRES_PASSWORD` fields in `cloudbeaver-db-env` variables. The password are empty by default and the service will not start without them.
