...
for svc in $TESERVICES; do
  ...
  docker tag cloudbeaver-"${svc}" ${AWS_ACC_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ENVIRONMENT}-cloudbeaver-"${svc}":${TEVERSION}
  docker push ${AWS_ACC_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ENVIRONMENT}-cloudbeaver-"${svc}":${TEVERSION}
 done